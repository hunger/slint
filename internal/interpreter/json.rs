        ...
        Value::Brush(brush) => match brush {
            ...
            Brush::LinearGradient(lg) => {
                Ok(serde_json::Value::Object(
                    vec![(
                        "gradient".to_string(),
                        serde_json::Value::from("linear")
                    ),
                    (
                        "angle".to_string(),
                        serde_json::Value::from(lg.angle()),
                    ),
                    (
                        "stops".to_string(),
                        serde_json::Value::Array(
                            lg.stops().map(|stop| serde_json::Value::Object(
                                vec![
                                    ("position".to_string(), serde_json::Value::from(stop.position)),
                                    ("color".to_string(), serde_json::Value::from(color_to_string(&stop.color)))
                                ].drain(..).collect::<serde_json::Map<_, _>>()
                            )).collect()
                        )                        
                    )].drain(..).collect::<serde_json::Map<_, _>>())
                )
            ...
            Brush::RadialGradient(rg) => {
                Ok(serde_json::Value::Object(
                    vec![(
                        "gradient".to_string(),
                        serde_json::Value::from("circle")
                    ),
                    (
                        "stops".to_string(),
                        serde_json::Value::Array(
                            rg.stops().map(|stop| serde_json::Value::Object(
                                vec![
                                    ("position".to_string(), serde_json::Value::from(stop.position)),
                                    ("color".to_string(), serde_json::Value::from(color_to_string(&stop.color)))
                                ].drain(..).collect::<serde_json::Map<_, _>>()
...
fn test_from_json() {
    ...
    assert!(value_from_json_str(&langtype::Type::Brush, "{ \"gradient\": \"linear\", \"stops\": [ 0.0 ] }").is_err());
    ...
    assert_eq!(
        v,
        Value::Brush(Brush::RadialGradient(i_slint_core::graphics::RadialGradientBrush::new_circle(vec![
            i_slint_core::graphics::GradientStop {
                position: 0.0,
                color: Color::from_argb_u8(0xff, 0xff, 0x00, 0x00)
            },
    ...
    assert!(value_from_json_str(&langtype::Type::Brush, "{ \"gradient\": \"circle\", \"stops\": [ 0.0 ] }").is_err());
}
...
fn test_to_json() {
    ...
    let v = value_to_json_string(
        &Value::Brush(Brush::RadialGradient(i_slint_core::graphics::RadialGradientBrush::new_circle(
            vec![
            i_slint_core::graphics::GradientStop {
                position: 0.0,
                color: Color::from_argb_u8(0xff, 0xff, 0x00, 0x00)
