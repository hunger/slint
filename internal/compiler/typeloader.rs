...
impl Snapshotter {
    ...
    pub(crate) fn create_component(
        ...
        assert!(!self.component_map.contains_key(&input_address));
    ...
    pub(crate) fn has_component(&self, component: &Rc<object_tree::Component>) -> bool {
        self.component_map.contains_key(&by_address::ByAddress(component.clone()))
    }
    ...
    pub(crate) fn create_element(
        ...
        assert!(!self.element_map.contains_key(&by_address::ByAddress(element.clone())));

        let elem = element.borrow();

        let r = Rc::new_cyclic(|weak| {
            let old_len = self.element_map.len();
            self.element_map.insert(by_address::ByAddress(element.clone()), weak.clone());
            assert_eq!(self.element_map.len(), old_len + 1);
