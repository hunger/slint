...
fn extract_color_from_evaluated_expression_impl(
    ...
    match evaluated_expression {
        ...
        {
            return extract_color_from_evaluated_expression_impl(from, palette_name, struct_level);
        }
            ...
            if source.element().borrow().base_type == langtype::ElementType::Global =>
        {
            ...
            if let Some(component_name) = elem.enclosing_component.upgrade().map(|c| c.id.clone()) {
        ...
        expression_tree::Expression::StructFieldAccess { base, name } => {
            ...
            return extract_color_from_evaluated_expression_impl(base, palette_name, struct_level);
        }
        ...
        expression_tree::Expression::CodeBlock(cb) => {
            ...
            return extract_color_from_evaluated_expression_impl(expr, palette_name, struct_level);
        }
        expression_tree::Expression::StoreLocalVariable { value, .. } => {
            return extract_color_from_evaluated_expression_impl(value, palette_name, struct_level);
        }
        _ => {}
    };
...
fn extract_color_from_expression_tree(
    ...
    extract_color_from_evaluated_expression_impl(expression, Vec::new(), 0).value
}
...
fn simplify_value(
    ...
    let eval_result = expression
        .as_ref()
        .and_then(|e| e.parent())
        .and_then(|e| eval_bindung_expression(document_cache, e.into()));
