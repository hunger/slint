...
export class PreviewWidget extends Widget {
    ...
    constructor(previewer: Previewer, _internal_url_prefix: string) {
        ...
        try {
            ...
            console.log("Caught expected throw when bringing up event loop", e);
        }

        setTimeout(async () => {
            console.log("CREATING PREVIEW_CONNECTOR");
            // Give the UI some time to wire up the canvas so it can be found
            // when searching the document.
            this.#preview_connector =
                await new slint_preview.PreviewConnector();
            await this.#preview_connector.show_ui();
            console.log(this.#preview_connector);
        }, 100);
